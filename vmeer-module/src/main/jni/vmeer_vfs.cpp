...
std::string RedirectPath(const char* original) {
    ...
    // 2. Redirect SDCard (Android 13/14 friendly)
...
int hook_getdents64(unsigned int fd, struct dirent64* dirp, unsigned int count) {
    ...
    int bpos = 0;
    while (bpos < nread) {
        struct dirent64* d = (struct dirent64*)((char*)dirp + bpos);
...
char* hook_getenv(const char* name) {
    ...
    return ((orig_getenv_t)shadowhook_get_prev_stack(reinterpret_cast<void*>(hook_getenv)))(name);
}
